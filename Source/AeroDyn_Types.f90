...
! =========  ElementParms  =======
  TYPE, PUBLIC :: ElementParms
    ...
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: Area      ! Cross-sectional area of elem [-]
...
ENDIF
IF (ALLOCATED(SrcelementparmsData%Area)) THEN
   i1_l = LBOUND(SrcelementparmsData%Area,1)
   i1_u = UBOUND(SrcelementparmsData%Area,1)
   IF (.NOT. ALLOCATED(DstelementparmsData%Area)) THEN 
      ALLOCATE(DstelementparmsData%Area(i1_l:i1_u),STAT=ErrStat)
      IF (ErrStat /= 0) THEN 
         ErrStat = ErrID_Fatal 
         ErrMsg = 'AD_Copyelementparms: Error allocating DstelementparmsData%Area.'
         RETURN
      END IF
   END IF
   DstelementparmsData%Area = SrcelementparmsData%Area
ENDIF
...
ENDIF
IF (ALLOCATED(elementparmsData%Area)) THEN
   DEALLOCATE(elementparmsData%Area)
ENDIF
...
ENDIF
 ...
 ! buffers to store meshes, if any
  ...
  IF ( ALLOCATED(InData%Area) ) THEN
    IF ( .NOT. OnlySize ) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%Area))-1 ) =  PACK(InData%Area ,.TRUE.)
 ...
 ! buffers to store meshes, if any
  ...
  ENDIF
  IF ( ALLOCATED(OutData%Area) ) THEN
  ALLOCATE(mask1(SIZE(OutData%Area,1))); mask1 = .TRUE.
    OutData%Area = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%Area))-1 ),mask1,OutData%Area)
  DEALLOCATE(mask1)
